...
function DrawLayer(selectionHandler) {
    ...
    this.select = new ol.interaction.Select({toggleCondition: ol.events.condition.never, style: this.style.styleFunctionSelect, condition: ol.events.condition.never});
    ...
    var loadElements = function(elements){
        ...
        _this.select.getFeatures().clear();
        if(elements!== null) {
            var features = new ol.source.GeoJSON({object: elements}).getFeatures();
            for(var i=0; i<features.length; i++){
                var feature = features[i];
                var geometry = feature.getGeometry().transform(getMercatorProjection(), _this.map.projectionFunction());
                feature.setGeometry(geometry);
